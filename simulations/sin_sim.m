% filepath: c:\Users\kavin\OneDrive\School\Winter_2025\ROB_599\auto-sailboat\simulations\sin_sim.m
clear; clc; close all;
...
Q = diag([100, 100, 0, 0, 0]); % State cost matrix (only penalize x and y)
R = diag([1, 1]); % Control effort cost matrix
...
% Define control input bounds
lb = repmat([-pi/4; -pi/4], prediction_horizon, 1); % Lower bounds for delta_r and delta_s
ub = repmat([pi/4; pi/4], prediction_horizon, 1); % Upper bounds for delta_r and delta_s

% Run the simulation with sinusoidal control inputs
for k = 1:N-1
    % Current state
    x = state(k, 1);
    y = state(k, 2);
    ...
    % Define sinusoidal control inputs
    delta_r(k) = (pi/8) * sin(0.1 * t(k));
    delta_s(k) = (pi/8) * cos(0.1 * t(k));
...
title('Sailboat Trajectory with Sinusoidal Control Inputs');
xlabel('X Position');
ylabel('Y Position');
